...
export async function insertBackup(clusterId: string, url: string) {
  const insertedId = await db.insertInto('backups').values({clusterId, url}).returning(['id']).executeTakeFirstOrThrow();
  console.log('Sample entry inserted with ID:', insertedId);
...
export async function getBackup(id: number | null) {
  ...
  const res = await db.selectFrom('backups').select('url').where('id', '=', id).execute();
  console.log(res)
  return res;
}
